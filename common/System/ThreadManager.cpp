            ...
            if (task) {
                if (task->state_ == ThreadManager::Task::EXECUTING) {
                    ...
                    try {
                        ...
                        LOG_DEBUG("task->run() raised an exception: [%s]",e.what());
                    } catch (...) {
                        //GlobalOutput.printf("[ERROR] task->run() raised an unknown exception");
                        LOG_DEBUG("task->run() raised an unknown exception");
                    }
...
void ThreadManager::Impl::start() {
    ...
    if (state_ == ThreadManager::UNINITIALIZED) {
        if (!threadFactory_) {
            //throw InvalidArgumentException();
            LOG_DEBUG("threadFactory is nullptr");
            return;
        }
...
void ThreadManager::Impl::add(std::shared_ptr<Runnable> value, int64_t timeout, int64_t expiration) {
    ...
    if (!g) {
        ...
        LOG_DEBUG("add task is timeout:%ld", timeout);
        return;
    }

    if (state_ != ThreadManager::STARTED) {
        LOG_DEBUG("ThreadManager::Impl::add ThreadManager not started");
        return;
    }
    ...
    if (pendingTaskCountMax_ > 0 && (tasks_.size() >= pendingTaskCountMax_)) {
        if (canSleep() && timeout >= 0) {
            ...
            LOG_DEBUG("Too Many Pending Tasks Exception");
            return;
        }
    }
...
void ThreadManager::Impl::remove(std::shared_ptr<Runnable> task) {
    ...
    if (state_ != ThreadManager::STARTED) {
        /*throw IllegalStateException(
            ...
            "started");*/
        LOG_DEBUG("IllegalStateException ThreadManager::Impl::remove ThreadManager not started");
        return;
    }
...
std::shared_ptr<Runnable> ThreadManager::Impl::removeNextPending() {
    Guard g(mutex_);
    if (state_ != ThreadManager::STARTED) {
        LOG_DEBUG("ThreadManager::Impl::removeNextPending ThreadManager not started");
        return NULL;
    }
